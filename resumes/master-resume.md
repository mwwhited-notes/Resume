...
- Designed a generic outbox abstraction with per-type DI-keyed routing, enabling pluggable outbox backends (in-memory, service bus, SSE, WebSocket) without changing event-producer call sites
...
- Created `dotnet new` platform scaffolding template generating a complete contract/implementation/test project set, reducing new module bootstrap to a single CLI command

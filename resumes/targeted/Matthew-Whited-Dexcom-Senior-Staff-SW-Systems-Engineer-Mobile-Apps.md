# Matthew Whited - Senior Staff SW Systems Engineer
## Connected Medical Device Architecture & Regulated Software Systems

---

## Contact Information
- **Name:** Matthew Whited
- **Email:** [matt@whited.us](mailto:matt@whited.us)
- **Phone:** [+1 (614) 989-1748](tel:+16149891748)
- **Location:** Baltimore, MD (Remote)
- **GitHub:** [github.com/mwwhited](https://github.com/mwwhited)
- **LinkedIn:** [linkedin.com/in/mwwhited](https://www.linkedin.com/in/mwwhited/)
- **Stack Overflow:** [22,510+ reputation](http://stackoverflow.com/users/89586/matthew-whited)
- **NuGet Profile:** [900K+ downloads](https://www.nuget.org/profiles/mwwhited/)

---

## Professional Summary

**Senior Staff Systems Engineer / Platform Architect** with over 20 years designing secure, regulated, cross-platform software systems — including 6+ years of direct FDA-compliant, HIPAA-adjacent medical device software development. Systems architect and technical lead for a distributed, multi-language clinical platform, owning cross-service security architecture, SDK/API design, and real-time data delivery across mobile, backend, and cloud consumers — the same architecture-across-mobile/backend/cloud/SDK/partner-integration scope this role is chartered to own for Dexcom's connected CGM ecosystem. Deep expertise in modern authentication and token-security standards directly applicable to securing connected medical devices. IC-track technical leader who drives cross-functional architecture decisions and mentors engineers, not a people-manager.

### Core Value Proposition for Dexcom
- **Direct Regulated Medical Device Domain Match:** 6+ years of FDA-compliant, design-controls, verification & validation (V&V) software development for clinical medical device systems and life-critical FDA-regulated systems for Big Pharma clients and a major academic medical research group — this is the same regulatory environment as Dexcom's CGM software, not an adjacent industry
- **Connected-Ecosystem Systems Architecture:** Owns cross-service security architecture and multi-target SDK/API design spanning mobile, backend, and cloud consumers — directly analogous to Dexcom's "architecture and design decisions across Mobile Applications, SDKs, Backend Services, Cloud Infrastructure, APIs, and partner integrations"
- **Security Architecture for Connected Devices:** Modern authentication and token-security implementation (OAuth 2.0/OIDC, proof-of-possession token binding) eliminating a class of token-replay attacks on sensitive clinical data — a near-exact match for "cybersecurity for connected medical devices"
- **Real-Time Connected-Device Experience:** Authenticated long-lived streaming connections for real-time data delivery — directly relevant to alerts, notifications, and data-sharing experiences across a connected device ecosystem
- **Cross-Functional Complex-Initiative Ownership:** Track record of owning and resolving highly complex, cross-functional technical initiatives end-to-end, from root-cause diagnosis through production-verified fixes

---

## Professional Experience

### Principal Software Engineer/Architect - Cadwell Industries (October 2025 - Present)
**Medical Device Software & Connected-Systems Architecture**

- Backend communication and synchronization software development for Cadwell/Cadlink medical device systems using WPF, .NET 4.8, SQLite, and WCF (named pipes and TCP for IPC/RPC)
- Technical leadership through peer code reviews, grooming sessions, and architectural guidance for FDA-regulated medical device software
- Architecture review and technical design input for the next-generation Cadwell Mesa platform as it scales

#### Cadwell Mesa — Next-Generation Modular Runtime (Connected Systems Architecture)
- Designed and delivered the platform's cross-service security and trust architecture, including modern OAuth 2.0/OIDC authentication and fine-grained, deny-by-default authorization enforced across distributed clinical services
- Designed a secure SDK architecture giving consumers across mobile, desktop, and server contexts consistent, built-in security guarantees with minimal integration overhead
- Built a code-generation pipeline producing typed client SDKs across multiple languages (.NET, TypeScript, Python) from a single API contract, with automated enforcement keeping client and server contracts in sync
- Implemented modern proof-of-possession token security, closing a class of token-replay vulnerabilities on sensitive clinical data access
- Implemented authenticated, long-lived streaming connections for real-time data delivery, directly analogous to alert/notification delivery in a connected-device ecosystem
- Delivered a cross-platform, multi-language module pattern supporting Go alongside .NET, demonstrating flexible, language-agnostic platform architecture
- Diagnosed and resolved a subtle concurrency defect causing intermittent platform health-check failures, then root-caused and fixed a related API contract-generation bug — both verified with dedicated automated test coverage

#### CadLink Platform Contributions
- Resolved critical security vulnerabilities in a HIPAA-regulated platform — removing hard-coded credentials, migrating secrets to configuration, and eliminating plaintext passwords from security event logs
- Redesigned the platform's user-identity handling to support concurrent multi-user access, removing a long-standing single-user-process limitation
- Improved the platform's clinical administration tooling across numerous contributions — client migration, status visibility, progress indicators — materially improving clinical IT operator experience

### Principal Solutions Architect - Green Onion (February 2025 - January 2026)
**Application Rescue & Modernization — Crisis Recovery Leadership**

- Reduced Azure monthly infrastructure costs from $1,500+ to under $350 (77% reduction, $13,800+ annual savings) while stabilizing a legacy application inherited from an offshore development transition
- Designed zero-downtime dual-API-client migration framework transitioning from monolithic to modular architecture with Azure B2C/MSAL authentication
- Implemented multi-tenant, role-based security architecture with automatic query scoping based on organizational access level

### Chief Solutions Architect - Out-of-Band Development, LLC (March 2015 - October 2025)
**Independent Technology Consulting**

- Delivered comprehensive solutions across .NET enterprise architecture, embedded systems integration, and multi-platform hardware/software bridging for client engagements
- Application architecture, reverse engineering, performance tuning, and technical training delivery

### Chief Technology Officer / Chief Solutions Architect - Itrica (November 2009 - July 2015)
**Life-Critical Clinical Trial Systems — FDA-Regulated Software**

- Built revolutionary clinical trial systems for Big Pharma clients (Sanofi, AstraZeneca, GSK) and Harvard Medical's TIMI Study Group, with direct FDA auditor interface responsibility
- Merit-based promotion to CTO for being recognized as the organization's most skilled senior architect
- Complete FDA compliance responsibility for life-critical systems, establishing the regulatory-engineering discipline that carries directly into Dexcom's CGM software environment

---

## Technical Excellence

### Systems Architecture & Connected Devices
- Microservices Architecture, Event-Driven Architecture, API Gateway Design, Contract-Based Module Systems — Expert level, direct multi-platform SDK/backend/cloud design experience
- Multi-target SDK generation (.NET, TypeScript, Python) from a single API contract — directly relevant to a connected ecosystem spanning mobile, backend, and partner integrations

### Security & Authentication (Connected-Device Relevant)
- **OAuth 2.0/OIDC:** Advanced — trust-model design and implementation for distributed services
- **JWT / Token-Based Authentication:** Advanced — token issuance, validation, and cross-service trust
- **Modern Token-Security Standards (proof-of-possession):** Proficient — closing token-replay attack vectors
- **FIPS 140-2:** Proficient — regulated-environment cryptographic compliance

### Real-Time & Streaming
- SignalR/WebSockets/SSE — Advanced, 8+ years, including authenticated long-lived connections for real-time delivery
- Real-time notification and alert delivery patterns from production systems

### Hardware-Software Integration
- Multi-platform hardware expertise (Arduino, ESP8266, STM32, ARM Cortex-M, PIC) with 10+ years embedded C/C++ — relevant systems-level fluency for a connected-device ecosystem spanning firmware-adjacent mobile/cloud integration
- Binary protocol reverse-engineering and multi-format protocol implementation

### Core Programming Languages
- **C# / .NET:** 20+ years, Expert — 900K+ NuGet downloads
- **Python:** 5+ years, Proficient — AI/ML pipelines, Flask endpoints
- **JavaScript/TypeScript:** 15+ years, Advanced — React frontends, Node.js tooling
- **Go:** 3+ years, Proficient — production platform module with OpenTelemetry/Prometheus
- **C/C++ (Embedded):** 10+ years, Proficient — Arduino, AVR, ARM Cortex-M

### Compliance & Regulated Industries
- **FDA Compliance:** Expert, 6+ years — Itrica CTMS, Cadwell medical devices
- **HIPAA:** Advanced, 6+ years — healthcare systems, PHI-safe API design
- **HL7 v2.3–2.5:** Proficient — patient data integration

---

## Business Impact & Results

- **6+ Years of FDA-Regulated Medical Device Software** spanning clinical trial systems, Big Pharma platforms, and connected clinical device synchronization — direct domain precedent for Dexcom's regulatory environment
- **77% Cloud Cost Reduction** ($13,800+ annual savings) through platform architecture optimization
- **900K+ NuGet Package Downloads** across a 42-package, 6-year-maintained ecosystem
- **22,510+ Stack Overflow Reputation** (Gold Badge, C#) with 600+ solutions reaching 4.7M+ developers
- **Zero-Downtime Migration Frameworks** and **Cross-Service Trust Architecture** delivered for production clinical systems handling PHI

---

## Education & Certifications

### DeVry University - Electronic Engineering Technology (2000 - 2002)
- Hardware systems foundation directly supporting the hardware-software bridge relevant to connected medical device architecture

### Eastland-Fairfield Career and Technical Schools - Electronics Technician Certification (1998 - 2000)
- Certified Electronics Technician — 25+ years of circuit design and electronics fundamentals

### Professional Certifications
- **CompTIA A+** and **CompTIA Network+** — infrastructure and network architecture foundation

---

## Why Dexcom

Dexcom's continuous glucose monitoring platform sits at the intersection of connected hardware, mobile applications, cloud infrastructure, and strict regulatory compliance — precisely the intersection this career has been built around, from clinical trial systems at Itrica to current platform-security and SDK architecture work in medical device software. The "Systems Engineering architect and technical lead" framing of this role — owning architecture and design decisions across Mobile Applications, SDKs, Backend Services, Cloud Infrastructure, APIs, and partner integrations — maps closely onto the platform-architecture work already delivered for a modular clinical software platform. The primary ramp-up area is Dexcom's specific mobile implementation stack (Swift/Kotlin/React Native); the systems-architecture and security-design fundamentals transfer immediately.

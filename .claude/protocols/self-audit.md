...
- **Mwwhited-BookLearning** - Personal learning-project org (high priority) — **cross-check against the local `C:\repo\_learning` submodule list before trusting the org page alone.** A WebFetch-based scan of this org's page on 2026-08-17 (`github_analysis_20260817.md`) found only 10 of the 15 repos that actually exist under this org — `PgPlayground`, `compuadd-450-bios`, `exp-.net-2.0-il-assembler`, `navtec-roms`, `pro-sql-server-2008-service-broker`, and `tektronix-2230` were missed, most likely a WebFetch pagination gap on the org's repo-listing page rather than those repos being private or newly created. Run `git submodule status` (or `ls`) against `C:\repo\_learning` and diff the resulting name list against whatever the org-page WebFetch returns — treat a mismatch as a signal to re-fetch with pagination/a `?page=2` param or fall back to the GitHub API, not as "the local copy must be stale." See `.claude/protocols/private-repo-audit-prompt.md`'s "Batch/Collection Audit" section for the fuller collection-review workflow this feeds into.

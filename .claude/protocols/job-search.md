...
- **Still apply all normal screening after scraping:** the script only finds title matches and pre-filters known exclusions — it does NOT check compensation. Run every "matched" company's actual comp against the listed-compensation-required rule, the $51K spread-ceiling rule, and the $200K floor before treating anything as a real candidate.

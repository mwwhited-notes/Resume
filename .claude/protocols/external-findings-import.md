...
#### Step 3.3a: Translation & Sensitivity Sweep (do this even if the audit content already looks generalized)
1. Extract every proper noun / codename appearing in the source audit file(s) (product names, component names, class/interface names).
2. Check each against `./SearchResults/Parameters/internal-name-translations.md`. Use the listed translation; add new entries for anything not yet listed, following that file's translation patterns.
3. Check the drafted bullets against `./SearchResults/Parameters/prohibited-names.md`'s vulnerability-disclosure guidance — rewrite anything that names a weakness class, describes a control-bypass mechanism, or implies a prior exploit.
4. **This sweep is not limited to the new bullets being added.** After Step 4.1 (below) makes the edit, `grep` the *entire* `resumes/master-resume.md` for every internal name involved in this import — section headers, the table of contents, and skills-table Evidence cells have all previously carried stale internal names forward from before a name was added to the translation table. Do not consider the import complete until that grep returns no matches.
...
- [ ] Full-document translation/sensitivity sweep passed (Step 3.3a — grep confirms no stale internal names anywhere in master-resume.md)
...
**For current examples of prohibited names and their safe replacements, read both (private — stored in SearchResults submodule):**
- `./SearchResults/Parameters/internal-name-translations.md` — authoritative lookup table of internal name → public-safe translation. Check every proper noun in a source audit against this table before writing a bullet, and add new entries here (don't improvise ad hoc phrasing) when a name isn't yet listed.
- `./SearchResults/Parameters/prohibited-names.md` — policy rationale, including a dedicated section on vulnerability/weakness-disclosure language (see Step 3.5 below)
...
### CRITICAL: Vulnerability/Weakness Disclosure Language

Beyond internal names, do not describe the *specific nature* of a security weakness that existed in a named, currently-shipping product at a **current employer** — even past-tense/"resolved" framing discloses it. This applies regardless of whether internal names have been generalized.

**Prohibited:** naming the weakness class ("hard-coded credentials," "plaintext passwords in logs"), describing a mechanism as circumventing a control ("bypasses session-based identity"), or implying a specific prior exploit ("eliminating replay attacks").

**Instead:** describe the outcome/practice — "conducted a security hardening pass... improving credential management practices," "binding tokens... for stronger anti-replay protection." See `prohibited-names.md`'s vulnerability-disclosure section for the full incident example this rule came from.
...
**Updated:** September 24, 2026 — Added the full-document translation/sensitivity sweep (Step 3.3a), the vulnerability/weakness-disclosure language rule, and the `internal-name-translations.md` lookup table, after an incident where a stale `prohibited-names.md` entry marked two internal product codenames "safe to use" and a generalization pass covered only new bullets, leaving the codenames live in headers/TOC/skills tables elsewhere in the document.
**Related:** `protocols/private-repo-audit-prompt.md`, `SearchResults/Parameters/internal-name-translations.md`, `SearchResults/Parameters/prohibited-names.md`
